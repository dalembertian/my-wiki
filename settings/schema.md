...
**Naming:** page filenames use Title Case matching the page's `# Heading` (e.g. `wiki/AI News/Some Page.md` with `# Some Page` as its first line, and no frontmatter — the filename and the `# Heading` are the only place the title lives). Prefer a concise, retrieval-friendly title over reproducing a source's full headline. The one permitted difference between the two is a character a filename can't hold: `:` in the heading becomes ` -` in the filename (`# Harness Engineering: Guides…` → `Harness Engineering - Guides….md`), and `?` is dropped (`# Is the AI Boom Real?` → `Is the AI Boom Real.md`). Wikilinks use the filename form. Clarified 2026-09-25, when a lint found such pages along with four whose headings carried a subtitle the filename didn't; subtitles belong in the body, not in the heading.
