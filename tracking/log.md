...
## [2026-09-23] ingest | Emergent Agent Dialects and the Oversight Problem

Unattended run, one source: the Guardian piece on Emergence's research into agents inventing their own dialect. Filed under `AI News` — reporting on a newly published finding, following the precedent set by [[Emergent Multi-Agent Collusion in OpenAI Evaluations]] and [[Claude's Computer-Checked Proof of Fermat's Last Theorem]] for research announcements, with `#agents` doing the topical work. Kept as one page (single thesis: emergent inter-agent language degrades oversight) and not merged into the collusion page, which reports a specific incident with its own timeline; the two argue different things and stay independently retrievable. Added a caveats section — press coverage of one lab's unpublished study, no sample size or opacity metric given, and the lab sells agent infrastructure.

Reciprocals written into [[Emergent Multi-Agent Collusion in OpenAI Evaluations]] (the hybrid agent-to-agent strings in that incident generalise here to cooperative agents with nothing to hide), [[The AI Revolution - From ANI to Superintelligence]] (a present-tense, sub-AGI instance of the "watching it won't tell you what it's doing" problem) and [[AI Digital Fossils in LLM Training Data]] (inverse case: human-origin nonsense fossilised into the corpus vs. vocabulary the models coin at runtime). `1.Index.md`, `3.Articles.md` and `4.History.md` updated; `tracking/ingested.md` line added and ticked.

## [2026-09-24] edit | Ingest queue moved to inbox/

New articles now arrive in `inbox/` instead of `source/`. Anything in `inbox/` is pending, so nothing has to be diffed against `tracking/ingested.md` any more. The final step of a successful ingest moves the article, and its `_assets/` folder if it has one, into `source/`. A skipped article stays in `inbox/`. `source/` is still never edited; the only write it gets is that move. `tracking/ingested.md` is now an append-only record: its header was rewritten and the `[x]` checkboxes removed from the existing lines. Updated `settings/schema.md` (Boundaries, Sources section, Bootstrapping, `tracking/ingested.md`, Ingest workflow, Lint workflow), `README.md` and `.gitignore` (`/inbox` added), and created `inbox/_assets/`.

## [2026-09-24] lint | ingested.md link fixes and source/ backlog check

Fixed four lines in `tracking/ingested.md` that linked to the raw file in `source/` instead of the wiki page it produced: Nine Emerging Developer Patterns for the AI Era, The Last Solo Programmers, Loop Engineering, and The Choices We Make About AI Now Are Critical. Checked `source/` against the record for the move to `inbox/`: two articles were never ingested, `LangGraph - Workflows and Agents.md` (it has an `_assets/` folder) and `Who's Who in Ai - AI World Journal Ai World.md`. The user will move them to `inbox/` by hand. Every line in the record matches a file in `source/`.

## [2026-09-24] ingest | Workflows vs. Agents - LangGraph's Pattern Catalog; Who's Who in AI - A Dated Roster

Interactive batch run, two articles, and the first ingest through `inbox/`. Both had been in `source/` without ever being ingested (found by the backlog check above) and the user moved them to `inbox/`. **LangGraph docs page** → `Building AI Agents`: the workflow-vs-agent distinction, built around the overview diagram's three tiers of autonomy. Covers both the Graph and Functional APIs and flags three problems in the example code (an `eval()` tool, uncapped evaluator loops, and the two prompt-chaining examples disagreeing). Reciprocals written into [[Six Python Agent Frameworks Compared]], [[LangChain's Middleware Model for Custom Agent Harnesses]], [[Function Calling and Tool Use in LLMs]] and [[Loop Engineering]]. **AI World Journal listicle** → `AI Companies & Initiatives`: kept as a roster that maps its ten names to deeper wiki coverage, with a section on why it's dated. Reciprocals written into [[Deep Learning Timeline (1982-2024)]], [[AlexNet and the Three Nonconformists Who Built the Deep Learning Boom]] and [[Demis Hassabis and DeepMind's Path to AGI]]. Neither was added as a spoke to a synthesis. The roster would fit [[The Origins of Generative AI]] by topic, but it adds no vantage point that isn't already there.

Catalogs updated (`1.Index.md`, `3.Articles.md`, `4.History.md`). Both articles moved to `source/`, with the LangGraph assets folder going to `source/_assets/`. No name collisions, `inbox/` is empty again, and both Sources links resolve.

## [2026-09-24] edit | Schema fixes from the first inbox/ ingest

Three corrections from the ingest above. The `tracking/ingested.md` format now documents the relative-path markdown links the file actually uses (the schema said `[[wikilinks]]`) and forbids linking to the article's own `source/` file, which was the error fixed earlier today. Ingest step 4 now describes the current `3.Articles.md` bullet-plus-`^block-id` format instead of the old "heading + paragraph". Step 5 now requires checking that the move actually happened, and the `inbox/` rule notes that an empty `_assets/` stays.

## [2026-09-24] ingest | Comparing Coding Agent Harnesses - Pi, Oh-My-Pi, OpenCode and Claude Code

Unattended run, one article: Akita's first-impressions review of Pi, Oh-My-Pi, OpenCode, Claude Code and Codex. Filed under `Building AI Agents` rather than `Building Software with Coding Agents` — the subject is the harness layer itself (tool registries, prompt routing, compaction and steering knobs, subagent behaviour), read at the source-code level, so it belongs with [[Agent Harnesses]] and [[Harness Engineering - Guides, Sensors, and Regulation Categories]] rather than with the dev-workflow pages. Kept as one page: the tool-inventory comparison, the two myth rebuttals (subagent parallelism, token-saving minimal prompts) and the context-window floor all serve the single thesis that harness choice, not model choice, is the variable — splitting them would separate the claims from their evidence. Retained a caveats section: default configuration, one pull request reviewed once per harness with the defect matrix given schematically, and version numbers (OMP v15.2.4, OpenCode 1.15.10) that date fast.

Reciprocals written into [[Agent Harnesses]] (the unpriced, terminal-side half of that market, plus the cleanest evidence for Agent = Model + Harness), [[Harness Engineering - Guides, Sensors, and Regulation Categories]] (the inner harness Fowler's guides and sensors sit on; the three-way review as probabilistic inferential sensing), [[Six Python Agent Frameworks Compared]] (same design one layer up, opposite sign on token cost), [[The Case for Markdown Skill Files Instead of MCP Servers]] (a harness betting deliberately on expensive tool schemas; the universal `read` as a knowledge-in-the-tool-prompt middle case) and [[Defending Against Destructive AI Agents]] (same author; permissive operation there assumes the recoverability layers here). No synthesis touched — unattended runs don't revise `wiki/_syntheses/`.
...
## [2026-09-25] ingest | Communication Discipline Beats Prompt Frameworks

Unattended run, one article: Akita's "Why LLMs Aren't Giving You the Result You Expect | Why I Prefer Claude Code Today". Filed under `Building Software with Coding Agents` — the subject is how a developer talks to a coding agent inside their own workflow (the agent is the thing being used), which puts it beside [[Structured Prompt-Driven Development (SPDD)]] rather than with the harness-internals pages. Kept as one page despite the two-part headline: the Claude Code vs. Codex aside is a short, explicitly subordinate section arguing the same thesis as the rest — the complaint is about the request or the harness, never the model — and splitting it out would leave a dated four-paragraph tool note with its argument removed. Page keeps the four-block structure (want / how / don't-want / validate), the ROM-consolidation worked example that makes the don't-want block concrete, the "set context first, then ask instead of prescribe" shift, and a caveats section noting that "never once" across 500+ hours is one practitioner's recall and that the article was itself written by Claude to the author's prompt.

Reciprocals written into [[Structured Prompt-Driven Development (SPDD)]] (the named opposite: same four blocks, disputing whether they belong in a governed artifact or in live conversation), [[Comparing Coding Agent Harnesses - Pi, Oh-My-Pi, OpenCode and Claude Code]] (same author, supplies the evidence for the harness-not-model claim and complicates it — no harness found every defect), [[Loop Engineering]] (opposite operating mode: designing a loop that runs unattended vs. refusing to leave the room, both landing on verification) and [[The Last Solo Programmers]] (craftsman side of that split, disagreeing about who is actually at risk). No synthesis touched — unattended runs don't revise `wiki/_syntheses/`.

`1.Index.md`, `3.Articles.md` and `4.History.md` updated; `tracking/ingested.md` line added. Article and its `_assets/` folder moved to `source/`; no name collision on either.

## [2026-09-25] ingest | Akita - How Do I Protect Myself From My Agents Deleting My Stuff? (merged into existing page)
English version of an article already ingested from its Portuguese original (*Como me precaver pros meus agentes não apagarem minhas coisas?*) into [[Defending Against Destructive AI Agents]]. Content matches section for section, so no new page: the article was added as a second `## Sources` bullet, and the "source is in Portuguese" note was changed to say that both language versions are cited. No summary change, so `3.Articles.md`, `1.Index.md` and `4.History.md` are untouched.

Added a reciprocal pair with [[Communication Discipline Beats Prompt Frameworks]] (same author: bad requests as the usual cause of agent disasters, and the recovery layers for when they happen anyway). Article and its `_assets/` folder moved to `source/`; no name collision on either.

## [2026-09-25] edit | Dropped the Portuguese duplicate of the destructive-agents article
At the user's request, removed `source/Como me precaver pros meus agentes não apagarem minhas coisas?.md` and its `_assets/` folder. The English version ingested earlier today is now the only source for [[Defending Against Destructive AI Agents]]. Removed its Sources bullet and the note about the two language versions, and removed its line from `tracking/ingested.md` so it only lists articles that exist. This is a user-authorised exception to "never delete from `source/`". `source/` is gitignored, so the files were moved to the vault's `.trash/` (and `.trash/_assets/`) rather than deleted outright.
...
## [2026-09-25] ingest | Team Structures for an Agentic World - Pyramid to Hourglass
New page in `AI Insights` from an AWS Events keynote by Stephen Brozovich, ingested one at a time. The clipped note holds only the talk blurb; the page is built from the 44-slide PDF deck in its assets folder. Checked for duplicates first: no match. Reciprocal links added to [[Why Most Enterprise Agentic Projects Are Doomed]] (the operating-model diagnosis), [[The Last Solo Programmers]] (deskilling), [[Cory Doctorow - The Reverse-Centaur Critique of AI]] (the "cut juniors" trap from the vendor side) and [[Defending Against Destructive AI Agents]] (enforcement outside the model).

`1.Index.md`, `3.Articles.md` and `4.History.md` updated; `tracking/ingested.md` line added. Article and its `_assets/` folder moved to `source/`; no name collision on either.

## [2026-09-25] lint | Heading/filename mismatches; synthesis check
Full structural pass over 47 pages: links, reciprocity, the `## Sources` rule, catalogs, block IDs, `tracking/ingested.md` against `source/`, asset folders, and `settings/categories.md` against the folders under `wiki/`. No broken links, no one-way links, no orphans, inbox empty. One finding: 10 pages whose `# Heading` didn't match the filename. Six differed only by characters a filename can't hold (`:` → ` -`, `?` dropped), which the Naming rule in `settings/schema.md` now permits explicitly. In the other four, the heading carried a subtitle that has now been trimmed: [[NVIDIA's Agent Infrastructure Bet]], [[Inherent Labs and Faraday]], [[Defending Against Destructive AI Agents]] and [[Loop Engineering]]. Noted but left alone: 34 tags are used on one page only, and [[Everyday ChatGPT Prompting Tips]] has a single inbound link. Content checks were made against the catalog summaries plus targeted reads of the pages in question, not a full re-read of every page. The synthesis check found one qualifying cluster (next entry).

## [2026-09-25] edit | New synthesis: Who Will Check the AI?
Created at the user's request after the lint. Picks up where [[The Harness Is the Product]] ends, with humans as the verifier of last resort, and argues that the ability to verify is built by doing the work AI absorbs. Seven spokes across four categories: [[The Last Solo Programmers]], [[Loop Engineering]], [[Structured Prompt-Driven Development (SPDD)]], [[Team Structures for an Agentic World - Pyramid to Hourglass]], [[Cory Doctorow - The Reverse-Centaur Critique of AI]], [[Communication Discipline Beats Prompt Frameworks]] and [[Mews's AI Agents for Hotel Front Desks]]. They agree on the diagnosis and disagree on the remedy. Reciprocal links were added on every spoke and on both related syntheses. Entries added to `2.Syntheses.md` and `1.Index.md`. This makes four syntheses in total.
